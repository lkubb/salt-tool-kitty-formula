...
We will see a quick setup to configure the ``tool_kitty`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_kitty`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_kitty`` formulas under ``/srv/salt/tool_kitty/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_kitty/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_kitty/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_kitty/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_kitty/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_kitty`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_kitty`` formulas under ``/srv/salt/tool_kitty/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_kitty/parameters/roles
...
We create a configuration for the role ``tool_kitty/server`` in ``/srv/salt/tool_kitty/parameters/roles/tool_kitty/server.yaml``:
...
We create another configuration for the role ``tool_kitty/client`` in ``/srv/salt/tool_kitty/parameters/roles/tool_kitty/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_kitty/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_kitty/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_kitty/parameters/dns:domain/  '/srv/salt/tool_kitty/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-kitty-as-json:
      test.nop:
        - name: {{ kitty | json }}
...
.. code-block:: sls
    ...
    kitty-config-file-file-managed:
      file.managed:
        - name: {{ kitty.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='kitty-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ kitty.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``kitty`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ kitty['winner'] }}
